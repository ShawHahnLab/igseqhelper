...
### Post-Module 2 islandSeqs refinement
#
...
# most abundant unique sequence per cluster.  That keeps it reasonably quick
# and simple for whole-repertoire clustering, but we can make more accurate
# consensus sequences for subsets of a repertoire (like relevant sequences for
# members of a lineage) with the original input FASTQ given to SONAR.
...
rule sonar_recluster_consensuses:
    ...
    log: WD_SONAR/"output/sequences/nucleotide/islandSeqs_recluster_{antibody_lineage}/log.txt"
    shell:
        """
            (
              date
              fastq_consensus.py {input} {output.csv} -O {output.details}
            ) | tee -a {log}
        """
...
rule sonar_module_3_collect:
    ...
    log: "analysis/sonar/{subject}.{chain_type}/longitudinal.{word}.{antibody_lineage}/log.collect.txt"
    shell:
        """
            (
              date
              echo "Running SONAR Module 3 merge_time"
              echo
              echo "Project directory: {params.wd_sonar}"
              echo "$(which sonar): $(sonar --version)"
              cd {params.wd_sonar}
              sonar merge_time {params.seqs} 2>&1
            ) | tee -a {log}
        """
