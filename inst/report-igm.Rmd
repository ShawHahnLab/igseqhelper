...
tbl <- subset(metadata$samples,
       grepl("IgM+", SpecimenCellType), select = c("SpecimenSubject", "Specimen", "Chain", "Type"))
keys <- paste(tbl$Specimen, tbl$Chain, tbl$Type, sep = ".")
germlines <- lapply(keys, function(key) {
  fp <- file.path(ROOT, "reporting", key, "germline.VDJ.aligned.fasta")
  if (file.exists(fp)) {
    dnar::read.fa(fp)  
  } else {
    list(name=c(), seq=c())
...
categorize_seqs <- function(txt, isolate_names, lineage_names) {
  fields <- c("IGHV", "IGHD", "IGHJ", "IGLV", "IGLJ", "IGKV", "IGKJ")
  bool_mat <- do.call(cbind, lapply(fields, function(field) {
    grepl(paste0("^", field), txt)
  }))
  idx <- apply(bool_mat, 1, function(row) match(TRUE, row))
  labels <- fields[idx]
  labels[txt %in% isolate_names] <- "AntibodyIsolate"
  labels[txt %in% lineage_names] <- "AntibodyLineage"
  labels
}

germlines <- do.call(rbind, lapply(1:length(germlines), function(idx) {
  if (length(germlines[[idx]]$name) > 0)
  data.frame(
    Specimen = tbl$Specimen[idx],
    Subject = tbl$SpecimenSubject[idx],
    Chain = tbl$Chain[idx],
    Type = tbl$Type[idx],
    SeqName = germlines[[idx]]$name,
    Seq = germlines[[idx]]$seq,
    Category = categorize_seqs(
      germlines[[idx]]$name,
      metadata$antibody_isolates$AntibodyIsolate,
      metadata$antibody_lineages$AntibodyLineage),
    stringsAsFactors = FALSE)
}))

germlines$Category <- factor(
  germlines$Category,
  levels = c(
    "AntibodyLineage", "AntibodyIsolate",
    "IGHV", "IGHD", "IGHJ", "IGHK", "IGLV", "IGLJ", "IGKV", "IGKJ"))

# Calc distance to each lineage sequence
germlines <- do.call(rbind, lapply(
  split(germlines, paste(germlines$Subject, germlines$Chain, germlines$Type)),
  function(chunk) {
  ref <- subset(chunk, Category == "AntibodyLineage")$Seq
  chunk$LineageDistance <- adist(chunk$Seq, ref)
  chunk
}))

# order entries
germlines <- germlines[order(
  germlines$Subject, germlines$Specimen, germlines$Chain, germlines$Category, germlines$LineageDistance, germlines$SeqName), ]

x <- (subset(germlines, Subject == "08N021" & Chain == "heavy"))
par(mar = c(2,2,2,8))
dnaplotr::plotDNA(x$Seq, groups = x$Category)

x <- (subset(germlines, Subject == "08N021" & Chain == "heavy"))
x <- do.call(rbind, lapply(split(x, x$Category), function(chunk) head(chunk, n = 6)))
par(mar = c(2,2,2,8))
dnaplotr::plotDNA(x$Seq, groups = x$Category)
