...
rule fastqc_reads:
    input: expand("analysis/fastqc/reads/{run}/Undetermined_S0_{rp}_001_fastqc.html", run=RUNS_FOR_IGSEQ, rp=["I1", "R1", "R2"])
...
def fastqc_setup_helper_rules():
    ...
    for run in RUNS_FOR_IGSEQ:
        ...
        rule:
            ...
            input: expand("analysis/fastqc/reads/{run}/Undetermined_S0_{rp}_001{suffix}", **args)
        rule:
